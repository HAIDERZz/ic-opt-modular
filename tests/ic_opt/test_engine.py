...
def test_single_testbench_single_corner(tmp_path):
    ...
    obs = evaluate(spec, [Point({"F": "24", "W": "0.8u"}, "user"), Point({"F": "30", "W": "0.6u"}, "user")], ex, store, deck=deck_for(spec))
...
def test_three_testbenches_three_corners_worst_case(tmp_path):
    ...
    obs = evaluate(spec, [Point({"F": "20", "W": "0.6u"}, "user")], ex, store, deck=deck_for(spec))[0]
...
def test_reuse_budget_and_rerun_is_continuation(tmp_path):
    ...
    first = evaluate(spec, [p1, p2], ex, store, deck=deck)
    again = evaluate(spec, [p2, p3], ex, store, deck=deck)          # p2 reused, p3 new (3rd simulation)
    ...
    with pytest.raises(BudgetExceeded, match="3 simulations used"):
        evaluate(spec, [p4], ex, store, deck=deck)
...
def test_retention_drops_psf_of_failed_runs_when_configured(tmp_path):
    ...
    obs = evaluate(spec, [Point({"F": "20", "W": "0.6u"}, "user"), Point({"F": "22", "W": "0.6u"}, "user")], ex, store, deck=deck_for(spec))
