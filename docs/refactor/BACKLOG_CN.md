...
| N-21 | **Windows 控制端无法导入网表**：Cadence Maestro 导出目录固有文件 `amap/__dspf_information__.`（文件名以点结尾，本机每个导出都有），Windows 普通路径会去掉尾点，`shutil.copytree` / `rmtree` / `open` 全部报"文件不存在"；受影响：`Deck.save`（decks 落盘）、`netlist.import` 的 `.staging` 清理、`render_netlist` 复制 bundle 到工作目录、`SshExecutor._put_tree` 本地打包（Windows tar 未验证） | N-15 Windows 验收发现（2026-09-26） | 已修（11b3482，`src/ic_opt/localpath.py` 的 `literal()`：Windows 上本地文件树操作一律走 `\\?\` 扩展路径，POSIX 不变；Windows 上 SSH 树传输的本地打包 / 解包改用 tarfile，拒绝越界成员；`.staging` 清理不再静默失败；169 定向测试绿）。Linux 上验不了 Win32 语义，**等 Windows 重跑 N-15 确认**（重跑说明 `ic-opt-accept/n15/N15_RERUN_WINDOWS_CN.md`） |
