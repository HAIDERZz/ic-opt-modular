...
| N-15 | N-5 的隔离冒烟脚本已绑入 site.yaml，但完整远程运行（实验室主机 + Spectre）未验证；ADR 已注明 | T16.7 | 进行中（2026-09-25，用户选方案 1）：控制端改为用户的 Windows / macOS 笔记本，经 OpenSSH 驱动本机（仿真服务器）。验收包 `<EDA_AI_AGENT>/ic-opt-accept/n15/`（工程、服务器本地基线 expected.json、`n15_compare.py`、`N15_PLAN_CN.md` 给笔记本 agent、报告模板）；等两份报告回传后由 Claude 核对 |
...
| N-19 | 单圈变压器表 Q@40 / Qs@40 两列在偏移带的误差 10–30%，B-12 / N-17 两轮补点都没改善（谐振附近 Q 陡，Q 列仍按原始尺寸直接建模） | 候选：给 Qp / Qs 曲线列加 `feature_map: xfm_bs_dimensionless`，或按 Qp_peak × 比值建模（`model: ratio` 需先允许 Q 列）；先用 T16.2a 的对照脚本评估 | 中，待拍板 |
