...
| N-20 | `xfm_query_verify.py` 整表体检串行单进程：多圈表两张 6.6 h（23 列 × 5 折 + k 列两种坐标对照 + SRF / 守卫 / 反向 / 示例；k_lf 全表联合拟合一组 15 min；本次只给 4 线程） | 候选：按表分进程并行；机器空闲时给 16 线程；加"只验指定列"参数（改哪几列验哪几列）；报告脚本的对照图不要重拟四列，直接读验证 JSON | 小，待拍板 |
