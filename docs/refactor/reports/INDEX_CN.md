...
| `XFM_QUERY_VERIFY_CN.html` | `xfm_query_report.py` | `xfm_query_verify_{bs,ms}_{ap,m10}.json`（`xfm_query_verify.py`） | [6MKZJ2QPf25CYocfPTJNZm](https://claude.ai/artifact/6MKZJ2QPf25CYocfPTJNZm) v3 | ms 两表为高频加密后的重验；2026-09-26 N-16（Lp/Ls 改 ratio + 无量纲坐标）后再次重跑 ms 两表（v4，单次 6.6 h；改前的两份留作 `xfm_query_verify_ms_{ap,m10}_20260925_direct.json`）：Lp@60 p90 1.53%→1.01% / 2.18%→1.32%，Ls@10 3.35%→1.94% / 3.67%→2.40% |
...
| `XFM_DENSIFY_CORNER_CN.html` | `corner_densify_report.py` | `corner_evaluation_xfm_bs_{ap,m10}.json`、`corner25_{signoff,candidates}_xfm_bs_{ap,m10}.json`（`<ic-opt-library>/n28_signoff/` 的 `make_corner.py` / `run_corner.sh` / `corner_evaluate.py`）；`corner_findings.json`；`figs/XFM_DENSIFY_CORNER_CN_{ap,m10}_test_points.png` | [3yXzTJjqcUTBCzynZuEaJ2](https://claude.ai/artifact/3yXzTJjqcUTBCzynZuEaJ2) | N-19 方案 3：角落定向补点两表各 25 点回流，N-17 测试点前后对比 |
| `XFM_BS_Q_RATIO_ACCEPTANCE_CN.html` | `xfm_bs_q_ratio_acceptance_page.py` | `xfm_bs_q_ratio_acceptance.json`（`xfm_bs_q_ratio_acceptance.py LIB SCRATCH OUT --variants direct,ratio,ratio_mapped,mapped`）；`figs/XFM_BS_Q_RATIO_ACCEPTANCE_CN_p90.png` | [4fZB8uePF7sP6iHp4gCEdz](https://claude.ai/artifact/4fZB8uePF7sP6iHp4gCEdz) | N-19 方案 2 验收：Q 列改 Q 峰值 × 比值，采用 |
