"""N-17 review page: the second densification round of the single-turn transformer tables -- sixty lib.densify picks per
table in the band center_spacing 8-24 um x OD_S/OD_P 0.8-1.25 through real EMX and adopted, ten independent random
designs per table as the test set (the B-12 method, both xfm_bs_ap and xfm_bs_m10).

Usage: n17_densify_report.py OUT_HTML   (reads n17_*_xfm_bs_{ap,m10}.json next to this script; figures go to figs/)
"""
...
ev = {s: json.loads((HERE / f"n17_evaluation_{s}.json").read_text(encoding="utf-8")) for s in STRATA}
picks = {s: json.loads((HERE / f"n17_densify60_candidates_{s}.json").read_text(encoding="utf-8")) for s in STRATA}
# Hand-written reading of the numbers (filled in after the run); the tables and figures below are data.
FINDINGS: dict[str, list[tuple[str, str, str]]] = json.loads((HERE / "n17_findings.json").read_text(encoding="utf-8")) \
    if (HERE / "n17_findings.json").exists() else {}
...
def figure(s: str) -> str:
    ...
    for ax, q in zip(axes, ["Lp@40", "SRF", "k@40"]):
        ...
        ax.bar([x - 0.2 for x in xs], b, width=0.4, color="#9a9c90", label=f"回流前（{e['rows_before']} 行）")
        ax.bar([x + 0.2 for x in xs], a, width=0.4, color="#2f6b4f", label=f"回流后（{e['rows_after']} 行）")
        ax.set_title(f"{s} · {q}：10 个独立测试点的相对误差")
        ax.set_xlabel("测试点")
...
<div class="kpis">
  <div class="kpi"><div class="v">{e['picks_signoff']['ok']}/{e['picks_signoff']['n']} + {e['test_signoff']['ok']}/{e['test_signoff']['n']}</div><div class="l">真实 EMX 成功（选点 + 测试点）</div></div>
  ...
  <div class="kpi"><div class="v">{pct(sm['SRF']['before']['max_rel'])} → {pct(sm['SRF']['after']['max_rel'])}</div><div class="l">SRF 测试点最大误差，回流前 → 后</div></div>
  <div class="kpi"><div class="v">{cov(e['picks_signoff']['per_quantity']['k@40']['coverage'])} / {cov(e['picks_signoff']['per_quantity']['SRF']['coverage'])}</div><div class="l">60 个选点上 k@40 / SRF 回流前区间覆盖（本应 95%）</div></div>
</div>
{findings(s)}
<h3>{n}.1 十个独立测试点：回流前 → 后</h3>
...
<header>
  ...
  <h1>单圈变压器第二轮补点复核（N-17）</h1>
  <p class="note">问题（BACKLOG N-17，B-12 的后续）：B-12 在同心到轻偏心子域补了 60 点后，偏移最大、两外径接近但不等的测试点反而变差。
  这一轮把 <code>lib.densify</code> 的池限定在中心偏移 8–24 µm、外径比（OD_S/OD_P）0.8–1.25 一带（score=ceiling，Lp@40 / Ls@40 / k@40 / SRF），
  每张表选 60 点；另外在同一带随机取 10 个与不确定度无关的点作独立测试集；xfm_bs_ap 与 xfm_bs_m10 各 70 点都过真实 EMX
  （ap 7 路、m10 5 路 × 8 线程 × 32 GB）。只回流 60 个选点，用 10 个测试点比较回流前后的预测。外径比不是单维边界，
  选点脚本把 densify 的候选池按比值过滤后再走原有的贪心选择（<code>&lt;ic-opt-library&gt;/n28_signoff/make_n17.py</code>）。</p>
</header>
{sections}
<h2>3 方法与数据</h2>
<ul>
  <li>误差 = 预测 / 实测 − 1（有符号）；回流前取自 lib_signoff 对测试点运行时的预测（模型按该表回流前的行训练），回流后取自回流并重拟后的 <code>lib.query</code>，k = 2。</li>
  <li>脚本：<code>make_n17.py</code>（选点）、<code>make_n17_test10.py</code>（测试点）、<code>run_n17.sh</code>（真实 EMX 与回流）、<code>n17_evaluate.py</code>（前后对比），均在 <code>&lt;ic-opt-library&gt;/n28_signoff/</code>；本页由 <code>n17_densify_report.py</code> 生成，数据副本 <code>n17_*_xfm_bs_{{ap,m10}}.json</code> 在本目录。</li>
</ul>
</div>
</body>
</html>
