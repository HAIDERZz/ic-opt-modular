"""XFM_BS_Q_MAP_ACCEPTANCE_CN.html: the N-19 option-1 acceptance (xfm_bs_q_map_acceptance.json) as a review page.

Usage: xfm_bs_q_map_acceptance_page.py OUT_HTML   (figure to figs/)
"""
...
out = Path(sys.argv[1])
rep = json.loads((HERE / "xfm_bs_q_map_acceptance.json").read_text(encoding="utf-8"))
TABLES = list(rep["tables"])
COLS = ["Qp@40", "Qs@40", "Qp@60", "Qs@60"]
DIMS = ["OD_P", "OD_S"]
VARIANTS = [("direct", "现状：直接建模"), ("mapped", "加无量纲坐标")]
...
def figure() -> str:
    ...
    for ax, t in zip(axes, TABLES):
        ...
        for i, ((v, name), color) in enumerate(zip(VARIANTS, ["#9a9c90", "#5b6fc9"])):
            ax.bar([x + (i - 0.5) * 0.38 for x in xs], vals[v], width=0.38, color=color, label=name)
        ax.set_xticks(xs)
...
def point_rows(t: str) -> str:
    ...
    m = {p["obs_id"]: p for p in e["mapped"]["points"] if p["source"] == "n17"}
    es = rep["tables"][t]["protocol_iii_tests"]["Qs@40"]
    ds = {p["obs_id"]: p for p in es["direct"]["points"] if p["source"] == "n17"}
    ms = {p["obs_id"]: p for p in es["mapped"]["points"] if p["source"] == "n17"}
    ...
    for v, name in VARIANTS:
        ...
        for c in COLS:
            ...
            rows.append(f"<tr><td>{name}</td><td>{c}</td><td class='num'>{mm['k_scale']:.2f}</td><td class='num'>{pct(mm.get('median_rel'))}</td>"
                        f"<td class='num'>{mm.get('coverage_2sigma_before', float('nan')):.2f}</td></tr>")
    return "".join(rows)
...
<div class="table-wrap"><table><thead><tr><th>结果列</th><th>留出的档</th><th class="num">留出行数</th><th class="num">现状：直接建模<br>中位 / <b>p90</b> / 最大</th><th class="num">加无量纲坐标<br>中位 / <b>p90</b> / 最大</th></tr></thead>
<tbody>{holdout_rows(t)}</tbody></table></div>
<h3>测过但从未回流的独立设计（完整库，校准后；中位 / 最大 / <b>2σ 覆盖</b>）</h3>
<div class="table-wrap"><table><thead><tr><th>结果列</th><th>测试集</th><th class="num">点数</th><th class="num">现状：直接建模</th><th class="num">加无量纲坐标</th></tr></thead>
<tbody>{test_rows(t)}</tbody></table></div>
<h3>N-17 偏移带的每个点：Q@40 误差与是否在区间内，现状 → 加坐标</h3>
...
<h3>完整库校准</h3>
<div class="table-wrap"><table><thead><tr><th>建法</th><th>结果列</th><th class="num">k_scale</th><th class="num">典型误差</th><th class="num">放大前 2σ 覆盖</th></tr></thead>
...
<div class="wrap">
<header>
  <div class="eyebrow">ic-opt-modular · 查询库 · xfm_bs_ap + xfm_bs_m10 · N-19 方案 1 · {html.escape(rep['date'])}</div>
  <h1>单圈变压器 Q 列加无量纲坐标的验收：没有采用</h1>
  <p class="note">问题（BACKLOG N-19）：Qp@40 / Qs@40 在中心偏移 8–24 µm 一带误差 10–30%，B-12 与 N-17 两轮补点都没改善。方案 1：只给 Qp、Qs 曲线列加
  <code>feature_map: xfm_bs_dimensionless</code>（Lp / Ls / k 已经用它），不改代码。验收在库的临时副本上做（<code>xfm_bs_q_map_acceptance.py</code>，
  {rep['fold_workers']} 个进程 × {rep['omp_num_threads']} 线程，{rep['wall_seconds']:.0f} 秒）：每张表 20 个外径档整档留出（80 折）；用今天的完整库（含 B-12、N-17 回流行）
  预测测过但从未回流的设计（ap：N-17 与 B-12 各 10 个，m10：N-17 的 8 个可用点）；完整库校准。</p>
</header>
<h2>结论</h2>
<div class="finding"><b>对 N-19 要解决的那一带，换坐标没有帮助，反而让区间变得过于自信，所以没有采用。</b>N-17 偏移带的 10 个点上（xfm_bs_ap）：Qp@40 中位误差 11.9% → 13.3%，区间覆盖 90% → 40%；Qs@40 8.6% → 11.5%，覆盖 90% → 60%。逐点看，误差只是换了符号（+18% → −26%、+31% → −26%），幅度没降。m10 表的 8 个点：Qp@40 中位 5.5% → 4.2%，但覆盖 100% → 75%。原因：在无量纲坐标里这些点看起来离已有行很近，模型因此很有把握，而 Q 在谐振附近还依赖绝对尺寸（经 SRF），这些坐标没把它表达出来——"更自信但不更准"，正是查询库最不该有的行为。</div>
<div class="finding ok"><b>换坐标对档与档之间的插值很有效，留作以后。</b>整档留出的 p90 误差从 30–45% 降到 6–19%（例如 ap 的 Qp@40 留出 OD_P 档 44.8% → 8.9%，Qs@40 43.9% → 6.6%）。这说明 Q 列在原始坐标下确实难插值；等偏移带补足行（或换成 Q 峰值 × 比值的建法）后，再连同坐标一起改，用同一套脚本验收。</div>
<div class="finding"><b>下一步的两条路。</b>方案 2：Q@f = Q 峰值（Qp_peak，平滑量）× 单独建模的比值，需要改 manifest 校验与组合模型（约半天），再验收。方案 3：在没吃透的角落（外径比 1.1–1.25 × 线宽 8–10 µm × 偏移 15–20 µm）定向补 20–30 点真实 EMX（约 15 分钟，需批准），因为两轮补点各只落了 3 / 2 个点在那里。两条路可以并行。</div>
<figure><img src="data:image/png;base64,{img}" alt="整档留出 p90 误差两种建法对比"><figcaption>两张表、四个 Q 列、留出 OD_P 或 OD_S 档：两种建法的 p90 相对误差。换坐标在这个指标上全面更好。</figcaption></figure>
{sections}
<h2>改动</h2>
<p>真实 manifest 没有改动；临时副本在 <code>&lt;ic-opt-library&gt;/n28_scratch_n19/</code>。数据 <code>xfm_bs_q_map_acceptance.json</code>，本页由 <code>xfm_bs_q_map_acceptance_page.py</code> 生成。</p>
</div>
</body>
</html>
"""
