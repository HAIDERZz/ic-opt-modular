"""N-19 option 2 (+1) acceptance on the N28 single-turn transformer tables (xfm_bs_ap, xfm_bs_m10): do the Q curve columns get
...
adopted rows included, since the test designs below were never adopted; spec.json and sims/ linked to the real store):
    ...
    ratio        : Qp / Qs: {model: ratio}  -- Q@f0 = Q peak model x a ratio GP on the dims (option 2)
    ratio_mapped : Qp / Qs: {model: ratio, feature_map: xfm_bs_dimensionless}  -- the ratio GP on the dimensionless inputs
    mapped       : Qp / Qs: {feature_map: xfm_bs_dimensionless}  -- option 1, for reference
...
Usage: xfm_bs_q_ratio_acceptance.py LIBRARY_ROOT SCRATCH_DIR OUT_JSON [--tables xfm_bs_ap,xfm_bs_m10] [--fold-workers 12]
       [--workers 4] [--variants direct,ratio,ratio_mapped,mapped]
Run with OMP_NUM_THREADS=2 and PYTHONPATH pointing at a source tree whose manifest allows model: ratio on Qp / Qs.
"""
...
CHANGES = {"direct": {}, "ratio": {"model": "ratio"}, "ratio_mapped": {"model": "ratio", "feature_map": MAPPED}, "mapped": {"feature_map": MAPPED}}
MIN_LEVEL_ROWS = 25
...
def main() -> None:
    ...
    ap.add_argument("--variants", default=",".join(CHANGES))
    args = ap.parse_args()
    t_start = time.time()
