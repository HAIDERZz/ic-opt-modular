...
<p>随机抽样本身在两轮试跑里暴露并修掉了 7 个问题（各自一个提交，均带回归测试）：① 端口间距外扩折线的直段没算进斜接偏移（AP 上间距短 0.33 µm）；② 巴伦嵌套逃逸的臂尖焊盘可以越出次级环平边；③ NT=2 紧凑规划器的车道上界没约束焊盘落在平边内（4% 悬空）；④ xfm_il 只查 P/S 之间的桥层间距，不查同一绕组自己的落点栈与相邻匝对角线（4/50 器件）；⑤ xfm_tw 的倒角阶梯没计入路径外轮廓的网格吸附，S 恰在间距下限时对角段短 2–3 nm；⑥ xfm_il 逃逸臂尖焊盘同②（10% 与 0.15% 悬空）；⑦ 自检脚本本身把过孔栈的中间层焊盘当成落点（误报，已排除）。修复后两个种子各重跑 300 个器件得到上面的数字。</p>
</div>
