...
| T16.2b 组合模型 | 变基中 | `Quantity.model: direct|ratio|resonance`、`library/composed.py`（ComposedGP：低频 × 理想谐振因子 × 残差，σ 独立相加，`posterior_cov` 同假设）、`Library.models` 先拟部件再拟组合列（每折重拟部件）、`lib.query` 的 `composition` | 真实库（scratch 副本，剔除回流行）：留一档 OD_P p90 2.91/2.94%（研究 DF 完全相同）、10 个格点间最大 4.75/9.24% 覆盖 100%；40 GHz 区域 robust 50→212 / 23→108，mean 4312→3748 / 3738→3117（新 SRF 模型在未采样的"偏心 + 两外径不等"处如实标不确定） |
