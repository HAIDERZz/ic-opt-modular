...
### 波次 2（2026-09-25 07:35 派发，~09:30 合入）

| 任务 | 提交（main） | 交付要点 | 验收 |
|---|---|---|---|
| T16.5 库组 | 8800ca5 R-20、3eeedbc R-21、b1ee958 R-22、7210790 N-4、a9c16dd N-7 | `Library(cache_dir=)` + 库根不可写时回退 `~/.cache/ic-opt/<哈希>`（库自身 `.cache` 仍被读取，只有新文件走回退）、各块 / CLI / 配方 `cache_dir=`；manifest `Quantity.rel_sigma_max`（不进缓存键；优先级 调用参数 > 按列 > 默认；块的 `rel_sigma_max` 默认改为 None）；`lib.region relax=`；校准 + 拟合持 `<校准文件>.lock`（无锁文件系统退回原行为）；region 回显 `k` 与按列上限，页面脚本据此措辞 | 新增 19 测试；非 pcell 366 绿；真实库六张表数据集键与模型文件不变（只读核对）；ruff 干净 |
| T16.6 测量组 | dfa8db5 R-24、1510fa4 R-25 | 曲线取值：样本上（相对 1e-9）逐位不变，其余频率一律在相邻样本间线性插值（插值"量"而非 S：合成 RLC 上 Qp 插值 S 偏 32–62%）；0 Hz–f1 之间 L、k 取首样本、Q 从 0 线性；扫频外报错；`dataset._row` 锚点低于起点只空该列。四端口默认拓扑（次级反相）写入文档；k_lf < −1e-6 时观测 `issues` 给出告警与可直接写入 spec 的 `topology` 行；数据集 `check` 新增 `negative_k_lf`（真实四张变压器表均为 0） | 六张表 9020 行 209,646 个值与基点逐位相同；G 门 6/6；非 pcell 377 绿；ruff 干净；真实库 `.cache` 清理到当前键 |
| T16.7 杂项 | 964dcd0 N-5、a7b9cdd N-6、9c8795a N-8、2776ab8 N-13 | ADR-0001 隔离冒烟脚本绑入只读 `site.yaml`（bwrap 本机验证到站点检查，完整远程运行未验证 → N-15）；pcell 注释去 em-opt 指针（代码 AST 不变）；报告脚本过 ruff，`ruff check` 裸命令覆盖 src/tests/reports（不再扫 vendor）；`position_in` 先按名字 M<n> 再按位置，生成器改传整数位置 | pcell 带 profile 980 绿、不带 346 绿；黄金 GDS 13/13；三套基线回放 0 差异；发现 N-14（几个旧报告脚本用退役字段名） |
| T16.2b 组合模型 | 442b0c3、34f04a5 | `Quantity.model: direct|ratio|resonance`、`library/composed.py`（ComposedGP：低频 × 理想谐振因子 × 残差，σ 独立相加，`posterior_cov` 同假设）、`Library.models` 先拟部件再拟组合列（每折重拟部件）、`lib.query` 的 `composition` | 真实库（scratch 副本，剔除回流行）：留一档 OD_P p90 2.91/2.94%（研究 DF 完全相同）、10 个格点间最大 4.75/9.24% 覆盖 100%；40 GHz 区域 robust 50→212 / 23→108，mean 4312→3748 / 3738→3117（新 SRF 模型在未采样的"偏心 + 两外径不等"处如实标不确定） |
