...
> **处理状态（2026-09-25）**：行 1–4、5、6、7、8、9（依赖声明；TuRBO 按用户选 b 从 `vendor/TuRBO` 安装）、10、11、12 已在 T15 修复
> （`docs/refactor/T15_NO_DEFAULT_RESOURCES_PLAN_CN.md` 第 8 节有提交号）；行 16（EMX 默认值）与 19（SSH 传输超时）也已在 T15.1 处理（资源字段必填、`transfer_timeout_s` 入 site.yaml）；
> 行 27–35 由 T15.7 文档清理处理；行 13–15、17、18、20–26 列入 T16（清单与优先级在 `BACKLOG_CN.md` 第 2 节）。本文其余内容保持审查当时的原样。
