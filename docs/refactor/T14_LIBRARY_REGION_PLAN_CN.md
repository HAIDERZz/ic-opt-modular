...
T14.4 验收已完成（见第 8 节）：C3 与原型逐项相同、P1 热 33/35 s、P2 冷 127/136 s、C4 60 GHz 跑通、C5 门 6/6；`lib.suggest` 输出改严格 JSON（3a8e477）。
...
## 8. 验收记录（2026-09-24，T14.4）与后续

- C3（`reports/library_query/region_acceptance.py`，结果 `region_acceptance_40g.json`）：与原型同网格（OD 2 / W 1 / CS 2 µm）下
  AP/M10 稳健 50 / 均值 4312、M10/M9 23 / 3739，各维范围与实测命中行逐项相同；正式版靠粗筛括号盒只算 33–39 万格（原型 160 万）。
- P1/P2：热跑 33 s / 35 s，冷跑 127 s / 136 s（6 进程拟合 93–100 s + 预测 28–30 s），`threads=8`；预算 3 / 6 min。
- C4（60 GHz，自动步长，含新校准各 ~165 s）：AP 稳健 1 / 均值 1762、实测命中 8；M10 30 / 2092、命中 5。页面
  `XFM_BS_60G_REGION_CN.html`；40 GHz 页 `XFM_BS_40G_REGION_CN.html` 由 block 输出重生成（`region_40g_*.json`）。
- C5：G1/G2 门 6/6——回流的 10 个签核行（非格点）使 09-23 冻结参考失效，按同一协议在当前 1048 行库重做基线，中点检查只配对
  格点值（b5d6278）；库测试、CLI 测试、`ruff` 全绿。
- **发现（不阻塞，记为后续）**：bs 两层的锚定量（L@40/60、SRF）GP 长度尺度很短（OD_S 0.07–0.08，OD_P 0.18–0.23），格点之间
  σ 5–9%，格点上 0.2–0.5%；低频量 Lp_lf 尺度长（格点间 σ 0.2%），电感库各量亦无此问题。因此 bs 的"稳健集"天然极小，
  均值集是实用答案，实测命中行作对照。候选改进：把 L@f0 分解为 L_lf × f(f0/SRF) 建模、或 bs 加密 OD 采样；先做几个格点间
  的真实 EMX 签核（`lib_signoff`）判断 σ 是否偏悲观——须用户批准。
- 其它后续：region 输出不回显 `k`（页面按默认 2σ 措辞）；`EXECUTION_PLAN_CN.md` 已记 T14。
