...
def plan_shape(spec: Spec, pipeline: list[Stage], corners, executor: Executor, parallel_jobs: int | None, site: Site | None) -> str:
    ...
    parts = [f"{tb} testbench sims" if tb else "", f"{dev} device sims" if dev else ""]
    ...
    return (f"({' + '.join(p for p in parts if p)}) = {len(children)} simulations per point on {executor.host}, "
