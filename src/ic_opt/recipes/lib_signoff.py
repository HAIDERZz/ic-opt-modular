...
``ic-opt run lib_signoff PROJECT library=<root> candidates=<json> [stratum=...] [top=10] [adopt=false] --plan``
...
def main(run: Run, *, library: str, candidates: str, stratum: str | None = None, top: int = 10, adopt: bool = False, k: float = 2.0) -> None:
    ...
    for part, pts in groups.items():
        spec = dataset._spec(lib.root / part).model_copy(update={"project": f"{run.spec.project}_signoff_{part}"})
