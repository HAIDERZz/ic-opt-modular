...
``ic-opt run lib_design PROJECT library=<library root> [stratum=...] budget=200 strategy=turbo top=5 [rel_sigma_max=0.15]``
...
processes sized by that entry, so no evaluation thread fits a model.
"""
...
def main(run: Run, *, library: str, stratum: str | None = None, budget: int = 200, batch: int = 20, strategy: str = "turbo",
         top: int = 5, seed: int = 0, k: float = 2.0, rel_sigma_max: float = domain.DEFAULT_SIGMA_REL_MAX) -> None:
    lib = query.Library(library, limits=run.site.host("local"))     # the controller's entry, not the executor host's
